...
const nationalParksArray = [
  ...
  // Populate options for location dropdown
  ...
  // Populate options for park type dropdown
  ...
  searchButton.addEventListener('click', function() {
    const locationValue = locationDropdown.value;
    const parkTypeValue = parkTypeDropdown.value;
    searchResults.innerHTML = '';

    // Perform search based on location
    if (locationValue) {
      const matchingParksByLocation = nationalParksArray.filter(park => park.State.includes(locationValue));
      displaySearchResults(matchingParksByLocation);
    }

    // Perform search based on park type
    if (parkTypeValue) {
      const matchingParksByType = nationalParksArray.filter(park => park.LocationName.includes(parkTypeValue));
      displaySearchResults(matchingParksByType);
    }
  });
  ...
  function displaySearchResults(parks) {
    if (parks.length > 0) {
      parks.forEach(park => {
        ...
        searchResults.appendChild(parkInfo);
      });
    } else {
